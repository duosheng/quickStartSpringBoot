...
public class GeneratorFacade {
     ...
     * @throws Exception
     */
	public void generateByClass(Class clazz,Class boClazz,String templateRootDir) throws Exception {
		new ProcessUtils().processByClass(clazz, boClazz,templateRootDir,false);
	}
	public void deleteByClass(Class clazz,Class boClazz,String templateRootDir) throws Exception {
		new ProcessUtils().processByClass(clazz, boClazz,templateRootDir,true);
	}
    ...
    public class ProcessUtils {
		...
		public void processByClass(Class clazz, Class boClazz,String templateRootDir,boolean isDelete) throws Exception, FileNotFoundException {
			Generator g = getGenerator(templateRootDir);
			GeneratorModel m = GeneratorModelUtils.newFromClass(clazz,boClazz);
			PrintUtils.printBeginProcess("JavaClass:"+clazz.getSimpleName(),isDelete);
			processByGeneratorModel(templateRootDir, isDelete, g, m);
		}
	...
	public static class GeneratorModelUtils {
		...
		public static GeneratorModel newFromClass(Class clazz) {
			...
			templateModel.put("boClazz", null);
			setShareVars(templateModel);
		...
		/**
		 * 设计上的失误,应该把bo,po放到一个javaClass里面
		 * @param clazz
		 * @param boClazz
         * @return
         */
		public static GeneratorModel newFromClass(Class clazz,Class boClazz) {
			Map templateModel = new HashMap();
			JavaClass poJavaClass = new JavaClass(clazz);
			JavaClass boJavaClass = new JavaClass(boClazz);
			poJavaClass.setBoClazz(boJavaClass);
			boJavaClass.setPoClazz(poJavaClass);
			templateModel.put("clazz",poJavaClass );
			templateModel.put("boClazz", boJavaClass);
			templateModel.put("boClassName",boClazz.getSimpleName());
			setShareVars(templateModel);

			Map filePathModel = new HashMap();
			filePathModel.put("boClassName",boClazz.getSimpleName());
			setShareVars(filePathModel);
			filePathModel.putAll(BeanHelper.describe(new JavaClass(clazz)));
			return new GeneratorModel(templateModel,filePathModel);
		}
